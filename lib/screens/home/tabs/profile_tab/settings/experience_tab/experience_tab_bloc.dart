...
class ExperienceTabBloc extends BaseBloc {
  ...
  Future<LanguageResponse> getLanguages() async {
    try {
      ...
      if (_cachedLanguages != null) {
        print(
            'ExperienceTabBloc: Возвращаю кэшированные языки - ${_cachedLanguages!.data.length} штук');
      ...
      print(
          '========== ExperienceTabBloc: Загрузка языков завершена ==========');
      return result;
    } catch (e, stackTrace) {
      print(
          '========== ExperienceTabBloc ERROR: Ошибка при загрузке языков ==========');
      print('Error: $e');
  ...
  Future<PackageTypesResponse> getPackageTypes() async {
    try {
      print(
          '========== ExperienceTabBloc: Начало загрузки типов упаковки ==========');

      // ✅ Возвращаем кэшированные данные если они есть
      if (_cachedPackageTypes != null) {
        print(
            'ExperienceTabBloc: Возвращаю кэшированные типы - ${_cachedPackageTypes!.data.length} штук');
      ...
      print('ExperienceTabBloc: API Response type: ${result.runtimeType}');
      print(
          'ExperienceTabBloc: Типы упаковки загружены - ${result.data.length} штук');
      ...
      print(
          '========== ExperienceTabBloc: Загрузка типов завершена ==========');
      return result;
    } catch (e, stackTrace) {
      print(
          '========== ExperienceTabBloc ERROR: Ошибка при загрузке типов ==========');
      print('Error: $e');
      print('StackTrace: $stackTrace');
      print('==========================================================');
      ...
      if (e.toString().contains('404')) {
        print(
            'ExperienceTabBloc: 404 ошибка - endpoint не найден на сервере, возвращаю пустой список');
